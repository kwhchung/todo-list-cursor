...
const TabManager: React.FC = () => {
  ...
  const fetchTabs = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/tabs');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingTab) {
        await axios.put(`http://localhost:8080/api/tabs/${editingTab._id}`, formData);
      } else {
        await axios.post('http://localhost:8080/api/tabs', formData);
      }
      fetchTabs();
      handleClose();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`http://localhost:8080/api/tabs/${id}`);
      fetchTabs();
    } catch (error) {
      // Error handling without console.error
    }
