...
const TagManager: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingTag) {
        await axios.put(`http://localhost:8080/api/tags/${editingTag._id}`, formData);
      } else {
        await axios.post('http://localhost:8080/api/tags', formData);
      }
      fetchTags();
      handleClose();
    } catch (error) {
  ...
  const handleDeleteClick = async (tag: Tag) => {
    try {
      const response = await axios.get(`http://localhost:8080/api/tags/${tag._id}/usage`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/tags/${tagToDelete._id}`);
      fetchTags();
